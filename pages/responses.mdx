...
| `input_image` | Image media input. Only valid on user messages. |
| `input_file` | File media input, including PDFs. Only valid on user messages. |
...
Use `input_image` with `image_url` for images:
...
Use `input_file` with `file_url` or `file_data` for files such as PDFs:
...
| HTTP status | `type` / `code` | Common causes |
| --- | --- | --- |
...
| 400 | `invalid_request_error` / `unsupported_input_type` | Unsupported content part type or media in a function-call output. |
| 400 | `invalid_request_error` / `unsupported_parameter` | Unsupported Responses compatibility field, such as `background`, `max_tool_calls`, non-empty `include`, `top_logprobs`, `parallel_tool_calls: false`, or non-default `truncation` / `service_tier`. |
